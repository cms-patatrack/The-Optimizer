...
pso = optimizer.MOPSO(objective=objective, lower_bounds=lb, upper_bounds=ub, num_particles=num_agents,
            ...
            , initial_particles_position='gaussian', default_point=default_point)

fig, ax = plt.subplots()
plt.scatter([p.position[0] for p in pso.particles], [p.position[1] for p in pso.particles], s=5)
