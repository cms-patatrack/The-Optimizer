...
class MOPSO(Optimizer):
    ...
    def parallel_coordinates(self, ax=None, highlighted_particle=None):
        return optimizer.plot._parallel_coordinates(self, ax=ax, highlighted_particle=highlighted_particle)
    
    def tight_plot(self, ax=None, label=None, plot_true_pareto=False, first_objective=None, second_objective=None, **kwargs):
        ...
        if plot_true_pareto:
            ...
            ref_x = reference_pareto[first_objective]
            ref_y = reference_pareto[second_objective]
        
        return optimizer.plot._tight_plot(ax=ax, x=x, y=y, ref_x=ref_x, ref_y=ref_y,
                           label=label, ref_label=true_pareto_label, **kwargs)
        
    
    def scatter(self, fig):
        """
        Scatter plot of the Pareto front.
        """
        if self.pareto_front is None or len(self.pareto_front) == 0:
            raise ValueError("Pareto front is empty. Run the optimization algorithm first.")
        fitnesses = np.array([particle.fitness for particle in self.pareto_front]).T
        return optimizer.plot._scatter(fig, fitnesses)
